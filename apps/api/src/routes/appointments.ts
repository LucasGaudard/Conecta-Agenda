...
export async function appointmentsRoutes(app: FastifyInstance) {
  app.get("/appointments", { preHandler: authenticate }, async (request, reply) => {
    ...
    if (!parsedQuery.success) {
      return reply
        .status(400)
        .send({ message: parsedQuery.error.issues[0]?.message ?? "Filtros invalidos." });
    }
    ...
    if (parsedQuery.data.date) {
      ...
      where.date = {
        ...
        ...(parsedQuery.data.to ? { lt: addDays(parseDateOnly(parsedQuery.data.to), 1) } : {}),
      };
    }
  ...
  app.get("/appointments/available-times", { preHandler: authenticate }, async (request, reply) => {
  ...
  app.get("/appointments/:id", { preHandler: authenticate }, async (request, reply) => {
    ...
    const appointment = await findOwnedAppointment(parsedParams.data.id, request.user.businessId);
    ...
    try {
      ...
      return tx.appointment.create({
        data: {
          ...
          endTime: addMinutesToTime(parsedBody.data.startTime, service.durationMinutes),
      ...
      return reply.status(500).send({ message: "Nao foi possivel criar o agendamento." });
    }

    return reply.status(201).send({ appointment: mapAppointment(appointment) });
  });

  app.put("/appointments/:id/status", { preHandler: authenticate }, async (request, reply) => {
    const parsedParams = appointmentParamsSchema.safeParse(request.params);
    ...
    if (finalAppointmentStatuses.has(existingAppointment.status)) {
      return reply.status(400).send({ message: "Este agendamento possui um status final." });
    }

    if (existingAppointment.status === parsedBody.data.status) {
      return reply.status(400).send({ message: "O agendamento ja possui este status." });
    }
  ...
  app.put("/appointments/:id/reschedule", { preHandler: authenticate }, async (request, reply) => {
    const parsedParams = appointmentParamsSchema.safeParse(request.params);
    ...
    if (
      ...
      return reply.status(400).send({ message: "Este agendamento nao pode ser remarcado." });
    }

    if (!existingAppointment.serviceId) {
      return reply.status(400).send({ message: "Servico do agendamento nao esta mais disponivel." });
    }
    ...
    const endTime = addMinutesToTime(parsedBody.data.startTime, existingAppointment.durationMinutes);
  ...
  app.delete("/appointments/:id", { preHandler: authenticate }, async (request, reply) => {
    const parsedParams = appointmentParamsSchema.safeParse(request.params);
    ...
    if (existingAppointment.status === AppointmentStatus.COMPLETED) {
      return reply.status(400).send({ message: "Um agendamento concluido nao pode ser cancelado." });
    }
