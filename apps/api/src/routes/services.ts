...
export async function servicesRoutes(app: FastifyInstance) {
  ...
  app.post("/services", { preHandler: authenticate }, async (request, reply) => {
    ...
    if (!parsedBody.success) {
      return reply
        .status(400)
        .send({ message: parsedBody.error.issues[0]?.message ?? "Dados do servico invalidos." });
    }
  ...
  app.put("/services/:id", { preHandler: authenticate }, async (request, reply) => {
    ...
    const existingService = await findOwnedService(parsedParams.data.id, request.user.businessId);
  ...
  app.patch("/services/:id/status", { preHandler: authenticate }, async (request, reply) => {
    const parsedParams = serviceParamsSchema.safeParse(request.params);
  ...
  app.delete("/services/:id", { preHandler: authenticate }, async (request, reply) => {
    const parsedParams = serviceParamsSchema.safeParse(request.params);
