...
import { customerBodySchema, customerParamsSchema, customersQuerySchema } from "../schemas/customers";
...
async function isWhatsappAvailable(whatsapp: string, businessId: string, customerId?: string) {
...
export async function customersRoutes(app: FastifyInstance) {
  ...
  app.get("/customers/:id", { preHandler: authenticate }, async (request, reply) => {
    ...
    const customer = await findOwnedCustomer(parsedParams.data.id, request.user.businessId);
  ...
  app.post("/customers", { preHandler: authenticate }, async (request, reply) => {
    ...
    if (!parsedBody.success) {
      return reply
        .status(400)
        .send({ message: parsedBody.error.issues[0]?.message ?? "Dados do cliente invalidos." });
    }
    ...
    if (!whatsappAvailable) {
      return reply.status(409).send({ message: "Ja existe um cliente com este WhatsApp." });
    }
    ...
    try {
      ...
      const customerWithStats = await findOwnedCustomer(customer.id, request.user.businessId);
      ...
      return reply.status(handledError.statusCode).send({ message: handledError.message });
    }
  });

  app.put("/customers/:id", { preHandler: authenticate }, async (request, reply) => {
    const parsedParams = customerParamsSchema.safeParse(request.params);
    const parsedBody = customerBodySchema.safeParse(request.body);
    ...
    const existingCustomer = await findOwnedCustomer(parsedParams.data.id, request.user.businessId);
    ...
    try {
      ...
      const customer = await findOwnedCustomer(existingCustomer.id, request.user.businessId);
